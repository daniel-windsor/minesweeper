...
function showCell (evt) {
  ...
  if (evt.target.classList.contains('mine')) {
    displayMessage('Oh Dear!')
    document.getElementById('mine-count').innerHTML = '<p>' + ("You are dead") + '</p>'
    revealMines()
    removeListeners()
    return
  }
  ...
  let box = document.getElementsByClassName('board')[0].childNodes

  let rand = Math.floor(Math.random() * 3)

  if(rand == 0) {
    box[idx].style.backgroundColor = "#d1dcff"
  } else if (rand == 1) {
    box[idx].style.backgroundColor = "#ffd1dc"
  } else {
    box[idx].style.backgroundColor = "#dcffd1"
  }
}
